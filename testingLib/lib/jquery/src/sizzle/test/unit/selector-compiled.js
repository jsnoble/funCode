...
test("element", function () {
	...
	var form, all, good, i, obj1, lengthtest, siblingTest, siblingNext, iframe, iframeDoc, html;
	...
	deepEqual(Sizzle("em + :not(:has(*)):not(:empty), foo", siblingTest), [siblingNext], "Non-qSA path correctly sets detached context for sibling selectors (jQuery #14351)");

	iframe = document.getElementById("iframe"), iframeDoc = iframe.contentDocument || iframe.contentWindow.document;
	iframeDoc.open();
	iframeDoc.write("<body><p id='foo'>bar</p></body>");
	iframeDoc.close();
	deepEqual(Sizzle("p:contains(bar)", iframeDoc), [iframeDoc.getElementById("foo")], "Other document as context");
...
test("XML Document Selectors", function () {
	...
	equal(Sizzle("elem:not(:has(*))", xml).length, 1, "Non-qSA path correctly handles numeric ids (jQuery #14142)");
});
...
test("broken", function () {
	...
	var attrbad,
	    broken = function broken(name, selector) {
		throws(function () {
...
test("pseudo - misc", function () {
	...
	deepEqual(Sizzle("[id='select1'] *:not(:last-child), [id='select2'] *:not(:last-child)", q("qunit-fixture")[0]), q("option1a", "option1b", "option1c", "option2a", "option2b", "option2c"), "caching system tolerates recursive selection");
...
test("pseudo - :lang", function () {
	...
	var docElem = document.documentElement,
	    ...
	    testLang = function testLang(text, elem, container, lang, extra) {
		var message,
		    full = lang + "-" + extra;
	    ...
	    assertMatch = function assertMatch(text, elem, selector) {
		ok(Sizzle.matchesSelector(elem, selector), text + " match " + selector);
	},
	    assertNoMatch = function assertNoMatch(text, elem, selector) {
	...
	ok(!Sizzle.matchesSelector(anchor, ":lang(en), :lang(en-us)"), ":lang does not look above an ancestor with specified lang");
	testLang("self", anchor, anchor, "es", "419");
	ok(!Sizzle.matchesSelector(anchor, ":lang(en), :lang(en-us), :lang(yue), :lang(yue-hk)"), ":lang does not look above self with specified lang");
	...
	ok(!Sizzle.matchesSelector(anchor, ":lang(en), :lang(en-us)"), "XML :lang does not look above an ancestor with specified lang");
	testLang("XML self", anchor, anchor, "es", "419");
	ok(!Sizzle.matchesSelector(anchor, ":lang(en), :lang(en-us), :lang(yue), :lang(yue-hk)"), "XML :lang does not look above self with specified lang");
...
test("caching", function () {
	...
	t("Deep ancestry caching in post-positional element matcher (jQuery #14657)", "#qunit-fixture a:lt(3):parent", ["simon1", "google", "groups"]);
});
...
test("select() with pre-compiled function", function () {
	...
	jQuery.each(["#qunit-fixture #first", "ol#listWithTabIndex > li[tabindex]", "#liveSpan1"], function (i, selector) {
